...
备份版本：`NRD-XXPROJECT-BACKUP-2026.06.26-r3.3.2`

> **当前最新版本 `r3.3.2`** ｜ 更新推送时间 **2026-06-26（UTC+8）** ｜ 提交 `c10b6ee` ｜ 位置：本仓库 `main` 分支。
> r3.3.2 在 r3.3.1 之上叠加信号审计前端封版（transition LLM 降级/旧版复核醒目告警、删除决策死代码、卡片时间 `Asia/Shanghai`、`evidence_raw_values.*` 可跳转、funding 取值优先 `last_rate`），后端 `demo/`、执行层等保持不变。
