...
版本标记：`NRD-XXPROJECT-BACKUP-2026.06.22-r3.0.1`
...
| 类型 | 路径 | 说明 |
| --- | --- | --- |
| FMZ 信号层本体 | `demo/最新交付物/neutral_regulation_demo_fmz.py` | v1.4.0，只读观察；生成短推、状态栏、`signal_review.jsonl`；内置 GEX info 适配、rank 展示、时区/前提耐久度、决策矩阵和服务器旁路 LLM 复核契约；不在 FMZ 主循环调用 LLM |
...
| 因子轻量卡 | `01_信号层_中性回路/因子文档/`、`02_执行层_Deribit/因子文档/`、`03_VRP门_建仓前定价/因子文档/`、`04_对冲模块/因子文档/` | 23 个当前运行因子 + 1 个 BIRL 候选评估均已补 `轻量因子卡`；BIRL 为 SHADOW，其余为 ACTIVE |

## 未正式启用资产

| 类型 | 路径 | 当前状态 |
| --- | --- | --- |
| 执行层 FMZ | `demo/最新交付物/spm_calendar_protected_short_v1.py` | STRATEGY_VERSION 2.7.0；默认 `ALLOW_ENTRY_TRADING=False`、`ALLOW_EXIT_TRADING=False`、`ALLOW_HEDGE_TRADING=False`、`ALLOW_TRADING=False`；仅作为空跑/后续测试交付物 |
...
| 路径 | 用途 |
| --- | --- |
| `00_总纲/中性回路工程总纲_v2026.06.22-r3.0.md` | 当前工程级主入口 |
...
```powershell
cd <local-gexmonitorapi-workspace>
