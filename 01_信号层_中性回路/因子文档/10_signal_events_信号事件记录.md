...
> 当前模块口径（r2.1 / 2026-06-19）：本页早期内容含 v1.1/v0.5 历史描述；当前 FMZ 信号层以 `demo/最新交付物/neutral_regulation_demo_fmz.py` 为准，版本 `demo_version=1.3.0`、`schema_version=nrd.schema.v1.0.0`。当前运行路径是短推 + `signal_review.jsonl` + materializer + 静态审计页；LLM 复核在服务器旁路层执行，不在 FMZ 主循环调用。
