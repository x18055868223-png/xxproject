...
| 字段 | 内容 |
|---|---|
| 因子 | NeutralRepair（DIE + Anchor 时序门） |
| 所属回路 | ① 信号层 · 中性回路 |
| 作用层 | 触发 / 时间先验 |
| 理论机制 | 将 M-DIE 单向位移事件与 Anchor 受损后修复状态合成为中性回路窗口是否打开的状态机。 |
| 预期符号 | 无交易方向符号；`episode_direction` 仅记录触发事件方向。 |
| 适用周期 | tick 级状态机，围绕事件冷却、锚受损、锚修复和确认窗口运行。 |
| 与现有因子重叠 | 依赖 M-DIE 与 Anchor，产物被 signal_events 与 EDB precondition 消费。 |
| 主要失效条件 | 无锚受损、锚修复不足、事件冷却未完成、事件过期或反向事件合并错误。 |
| 改变的决策 | 改变 `nr_active`、信号窗口是否成立，以及是否生成审计卡。 |
| 当前状态 | ACTIVE |
