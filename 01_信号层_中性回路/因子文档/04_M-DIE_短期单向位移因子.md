...
| 所属回路 | ① 信号层 · 中性回路 |
| 作用层 | 触发 |
| 理论机制 | 用 15 分钟价格位移、路径效率和方向持续性判断是否出现干净的短期单向再定价事件。 |
| 预期符号 | 正值为 UP 事件，负值为 DOWN 事件；事件方向不是交易方向。 |
| 适用周期 | 1m K 线最近 15 根已收盘 bar。 |
| 与现有因子重叠 | 只喂给 NeutralRepair 作为 DIE 事件源，不参与 EDB 方向投票。 |
| 主要失效条件 | bar 不足、未来 bar 或重复 bar 混入、震荡漂移导致路径效率失真。 |
| 改变的决策 | 改变 NeutralRepair episode 创建、合并、冷却与窗口触发进度。 |
| 当前状态 | ACTIVE |
