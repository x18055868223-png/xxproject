...
| 所属回路 | ② 执行层 · Deribit |
| 作用层 | 审计 / 风险门 |
| 理论机制 | 对候选垂直结构计算 win_rate、rr、credit、margin/plan 指标并形成可复核的方案库排序。 |
| 预期符号 | 综合分越高表示候选更优；不产生市场方向。 |
| 适用周期 | PLAN 轮菜单生成与复核。 |
| 与现有因子重叠 | 消费 leg_selection、S:PM、VRP 与信号置信输入；VRP edge 分开展示，不并入软排序权重。 |
| 主要失效条件 | 报价过期、费用/价差假设错误、KPF 残留权重未清、plan_ev 被误读为真实期望收益。 |
| 改变的决策 | 改变方案展示、排序、候选编号和 ORDER 轮可复核对象。 |
| 当前状态 | ACTIVE |
