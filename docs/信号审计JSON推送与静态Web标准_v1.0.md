...
```json
{
  ...
  "delivery": {
    "fmz_push_summary": "【信号】BTC #3e3b 中性/等待确认 强度61 置信38未校准 冲突38% 同TMV+1.00,CVD+0.55 反MACRO-0.25,SRD-0.08 审计:a.example/c/3e3b",
...
强度<strength> 置信<confidence><校准状态> 冲突<ratio>
...
```text
【信号】BTC #3e3b 中性/等待确认 强度61 置信38未校准 冲突38% 同TMV+1.00,CVD+0.55 反MACRO-0.25,SRD-0.08 审计:a.example/c/3e3b
```
...
| 校准状态 | `decision.confidence_calibration` |
| 冲突 | `conflict.ratio` |
