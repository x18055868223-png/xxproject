...
- The current deployable signal-layer artifact is `demo/最新交付物/neutral_regulation_demo_fmz.py`. Its verified in-file version is `demo_version = "1.5.2"` and `schema_version = "nrd.schema.v1.0.0"`, with producer-native `SignalDurabilityLayer@nrd.signal.durability_layer.v1`, `SignalPriceAnchorDurability@nrd.signal.price_anchor_durability.v1`, and `SignalComfortWindow@nrd.signal.comfort_window.v1`.
...
- Signal v1.5.2 uses full local JSON audit records, full `SignalSessionPremiseDurabilityContext` time-zone/premise durability fields, `decision_matrix.temporal_durability`, native transition producer anchors, MACRO dual-axis shock-gate audit fields, an AUDIT_ONLY `signal_durability` total layer with comfort-window and price-anchor sublayers, a short FMZ push summary containing compact durability tokens, and out-of-process LLM sidecar review tooling. Runtime recording writes through `JsonlRecorder` to `demo/logs/<name>.jsonl`; the signal review recorder name is `signal_review`.
...
- Signal audit v1.5.2 remains read-only/observability-bounded: full JSON record, full time-zone/premise durability context, local JSONL, decision matrix temporal mirror, transition anchor, MACRO shock-gate audit fields, AUDIT_ONLY `signal_durability`, and short push summary must not touch execution-layer trading switches. MACRO direction score remains a direction/background vote; only confirmed `macro_shock.block` can create `MACRO_SHOCK_BLOCKING`.
