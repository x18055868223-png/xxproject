...
- The current deployable signal-layer artifact is `demo/最新交付物/neutral_regulation_demo_fmz.py`. Its verified in-file version is `demo_version = "1.5.1"` and `schema_version = "nrd.schema.v1.0.0"`.
...
- The current transition LLM review sidecar path in `tools/gemini_signal_llm_review.py` uses `gemini_signal_transition_review_prompt@1.2.4` and `signal_transition_llm_review@1.2.4`; content-expression issues are metadata/reference signals and do not gate display by themselves.
- The remote full-project backup branch for the current r3.3.5 asset state is `codex/full-project-backup-r3.3.5` on `xxproject`; it keeps the full `xxproject/main` project tree and layers the r3.3.5 signal-audit changes plus the current `demo/最新交付物/` signal/manual-gate FMZ deliverables.
...
- Signal v1.5.1 uses full local JSON audit records, full `SignalSessionPremiseDurabilityContext` time-zone/premise durability fields, `decision_matrix.temporal_durability`, native transition producer anchors, MACRO dual-axis shock-gate audit fields, a short FMZ push summary, and out-of-process LLM sidecar review tooling. Runtime recording writes through `JsonlRecorder` to `demo/logs/<name>.jsonl`; the signal review recorder name is `signal_review`.
...
- The deploy signal-audit frontend baseline for r3.3.1 as of 2026-06-25 is the compact reader-style audit page with transition audit, mandatory LLM review/pending section, MACRO/GGR auxiliary semantics, `source_ref` anchors back to raw trace groups, production/default materializer output excluding synthetic/local preview cards, strict session-context deploy acceptance, and MACRO direction-background / shock-gate display inside the existing MACRO evidence row.
...
- Verified during 2026-06-25 r3.3.1 local closure: producer, materializer, deploy-asset, frontend, LLM sidecar, and server-bootstrap tests are the required release gate with Python 3.12. New-server signal-stack bootstrap lives at `tools/server_bootstrap_signal_stack.sh`, with migration guidance in `deploy/signal_audit/SERVER_MIGRATION.md`; deployment acceptance must verify the latest real card has `identity.strategy_version=1.5.1`, native session context, native transition anchor/context, and producer-native `factor_cross_section.macro_pressure.macro_shock`.
...
- Signal audit v1.5.1 remains read-only/observability-bounded: full JSON record, full time-zone/premise durability context, local JSONL, decision matrix temporal mirror, transition anchor, MACRO shock-gate audit fields, and short push summary must not touch execution-layer trading switches. MACRO direction score remains a direction/background vote; only confirmed `macro_shock.block` can create `MACRO_SHOCK_BLOCKING`.
...
- If the API usage page shows zero Gemini calls after new signals, treat that as a deployment/config fault until proven otherwise. First verify `/etc/signal-audit/llm.env` has at least one Gemini channel key loaded: `GEMINI_CHANNEL1_API_KEY` for low-cost/free first pass and `GEMINI_CHANNEL2_API_KEY` for paid fallback. Then run `LLM_REQUIRED=1 SESSION_CONTEXT_REQUIRED=1 TRANSITION_REQUIRED=1 TRANSITION_LLM_REQUIRED=1 tools/server_self_check_signal_stack.sh --run-oneshots`; r3.3.1 self-check must fail when the latest signal card lacks an OK two-call sidecar review with `blind_review_mode=two_call_strict` and `llm_call_count>=2`, lacks the native full session-context schema, is materializer-backfilled, has a stale `identity.strategy_version`, or lacks producer-native `macro_shock`. Sidecar `api_key_route` / `llm_call_routes` shows whether the review used channel 1, channel 2, or mixed routing.
