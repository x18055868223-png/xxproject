...
- The current deployable signal-layer artifact is `demo/最新交付物/neutral_regulation_demo_fmz.py`. Its verified in-file version is `demo_version = "1.4.1"` and `schema_version = "nrd.schema.v1.0.0"`.
...
- Signal v1.4.1 uses full local JSON audit records, full `SignalSessionPremiseDurabilityContext` time-zone/premise durability fields, `decision_matrix.temporal_durability`, a short FMZ push summary, and out-of-process LLM sidecar review tooling. Runtime recording writes through `JsonlRecorder` to `demo/logs/<name>.jsonl`; the signal review recorder name is `signal_review`.
...
- The deploy signal-audit frontend baseline is r3.2.1 as of 2026-06-24: compact EDB evidence ledger, `source_ref` anchors back to raw trace groups, raw trace grouped navigation, mandatory LLM review/pending section, MACRO/GGR auxiliary semantics, production/default materializer output excluding synthetic/local preview cards, and strict session-context deploy acceptance.
...
- Verified during 2026-06-24 session-context closure: producer, materializer, deploy-asset, frontend, LLM sidecar, and server-bootstrap tests pass with Python 3.12. New-server signal-stack bootstrap lives at `tools/server_bootstrap_signal_stack.sh`, with migration guidance in `deploy/signal_audit/SERVER_MIGRATION.md`; the bootstrap release default is `r3.2.1`.
...
- Signal audit v1.4.1 changes are observability-only: full JSON record, full time-zone/premise durability context, local JSONL, decision matrix temporal mirror, and short push summary must not change direction, EDB score, confidence, blocking, or execution contracts.
...
- New-server rebuilds should start from `xxproject` release tags using `tools/server_bootstrap_signal_stack.sh`; for the current migration asset use `r3.2.1`. The script creates env templates only; `/etc/signal-audit/llm.env`, `/etc/gexmonitorapi.env`, FMZ runtime JSONL, and historical sidecar JSONL remain server-local and must not be committed.
- If the API usage page shows zero Gemini calls after new signals, treat that as a deployment/config fault until proven otherwise. First verify `/etc/signal-audit/llm.env` has at least one Gemini channel key loaded: `GEMINI_CHANNEL1_API_KEY` for low-cost/free first pass and `GEMINI_CHANNEL2_API_KEY` for paid fallback. Then run `LLM_REQUIRED=1 SESSION_CONTEXT_REQUIRED=1 tools/server_self_check_signal_stack.sh --run-oneshots`; r3.2.1 self-check must fail when the latest signal card lacks an OK two-call sidecar review with `blind_review_mode=two_call_strict` and `llm_call_count>=2`, lacks the native full session-context schema, is materializer-backfilled, or has a stale `identity.strategy_version`. Sidecar `api_key_route` / `llm_call_routes` shows whether the review used channel 1, channel 2, or mixed routing.
