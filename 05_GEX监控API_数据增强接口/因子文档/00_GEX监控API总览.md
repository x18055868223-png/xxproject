...
> 当前模块口径（r2.2 / 2026-06-19）：当前服务版本 `gexmonitorapi=0.2.1`，权威接口为 `/v1/info`。本模块是信号层的只读外部上下文增强，不决定方向、不进入交易授权、不解锁执行层。
...
- `quality=warming_up` 表示 rank 窗口覆盖尚未满 15 天或冷启动，不代表数值错误；满 15 天直接为 `ok`，但分位计算仍只看最近 30 天窗口。
...
1. 保持 `/v1/info` 合同稳定。
2. 继续积累 rank 历史样本，满 15 天后质量为 `ok`；满 30 天后仍保留全量历史，但当前分位只用最近 30 天。
