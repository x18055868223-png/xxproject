...
## Summary
在本地 GEX Monitor API 工作区新建一个 Python/FastAPI 服务，用 Scrapling 抓取并结构化 GEX Monitor 公开分析页面的可见内容。第一版只覆盖 BTC 的四个分析子页：`gex`、`gamma`、`volatility`、`flow`。服务本地 Windows 调试通过后，部署到 AWS 轻量 Linux 服务器，使用单个 Bearer token 保护摘要和刷新端点。
