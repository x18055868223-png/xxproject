...
def test_local_tail_acceleration_can_drive_hedge_ready_without_external_context():
    entry = H.build_entry_risk_anchor(
        direction_bias="SHORT_CALL",
        entry_price=73000,
        entry_dte_hours=48,
    ...
    assert pkg["tail_risk_state"] == H.STATE_HEDGE_READY
    assert "LOCAL_TAIL_ACCELERATION" in pkg["reason_codes"]


def test_persistence_two_item_edb_ggr():
    """持续性两项制 {EDB_ADVERSE, GGR_ADVERSE}；0→LOW/1→MEDIUM/2→HIGH。"""
    ...
    assert s2 == H.PERSISTENCE_HIGH and sorted(c2) == ["EDB_ADVERSE", "GGR_ADVERSE"]
