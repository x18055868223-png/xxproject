...
W = {"win_rate": 0.375, "rr": 0.375, "manual": 0.25}
...
def test_delta_fit_and_preferred_delta():
    pd_low = P.plan_preferred_delta(30, (0.15, 0.45))
    pd_high = P.plan_preferred_delta(90, (0.15, 0.45))
    assert pd_high > pd_low                             # 人工审计强 → 偏高 delta
    assert _approx(P.plan_delta_fit(0.30, 0.30), 1.0)
    assert P.plan_delta_fit(0.30, 0.20) < 1.0
...
def test_plan_rank_applies_execution_feasibility_penalty():
    high_surface_fragile = {"name": "fragile", "qualified": True, "win_rate": 0.90,
                            "rr": 1.0, "delta_fit": 1.0,
                            "execution_feasibility_penalty": 0.50, "tags": []}
    lower_surface_buildable = {"name": "buildable", "qualified": True, "win_rate": 0.70,
                               "rr": 0.8, "delta_fit": 0.8,
                               "execution_feasibility_penalty": 1.0, "tags": []}
    menu = P.plan_rank([high_surface_fragile, lower_surface_buildable], W, 2)
    assert menu[0]["name"] == "buildable"
    assert high_surface_fragile["surface_composite"] > lower_surface_buildable["surface_composite"]
    assert high_surface_fragile["composite"] < lower_surface_buildable["composite"]
