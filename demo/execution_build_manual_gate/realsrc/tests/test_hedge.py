...
def test_signed_target_opposite_direction_unwinds_before_reopen():
    a = H.hedge_order_action(-50.0, 100.0, 1)
    assert a["action"] == "HEDGE_UNWIND"
    assert a["reduce_only"] is True
    assert a["delta_contracts"] == 50.0
