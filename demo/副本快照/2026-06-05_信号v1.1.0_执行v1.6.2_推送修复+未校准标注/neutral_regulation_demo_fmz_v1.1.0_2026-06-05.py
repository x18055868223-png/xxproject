...
CONFIG = {
    ...
    # honesty tag. schema_version stays v1.0.0 deliberately: calibration_state is
    # an ADDITIVE, non-breaking field, so the SignalEvidencePackage wire contract
    # is unchanged and no consumer/schema rev is required.
    "demo_version": "1.1.0",
...
def render_review_card_push(card):
    ...
    lines.extend(["◎ 置信链", _confidence_chain_line(decomp, blocking, cal), ""])
...
def _conclusion_sentence(conclusion):
    ...
    if support in ("TRADE_SUPPORT_STRONG", "TRADE_SUPPORT_WEAK"):
        ...
        action = "暂不交易"
    cal = conclusion.get("calibration_state") or "UNCALIBRATED"
    cal_note = "（未校准）" if cal != "CALIBRATED" else ""
    return "{0}、{1}，置信 {2}/100{3}。".format(
        lean_cn, action, _fmt_int(conclusion.get("confidence")), cal_note)
...
def _confidence_chain_line(decomp, blocking, calibration_state="UNCALIBRATED"):
