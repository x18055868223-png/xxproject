...
def fmz_push(text):
    """Log with FMZ push. A message ending in '@' enters the platform push queue
    (app / email / etc. per the FMZ account push settings). Only the last push in
    a 20s live cycle is delivered, so push at most one consolidated message per
    signal event. Falls back to print() off-platform."""
    log_func = globals().get("Log") or getattr(builtins, "Log", None)
    if callable(log_func):
        log_func(str(text) + "@")
    else:
...
# --------------------------------------------------------------------------
# reading-optimized push 综述 (Layout 1 · sectioned narrative)
# --------------------------------------------------------------------------
...
def render_review_card_push(card):
    ...
    lines = [
        "【中性回路·信号审计】" + utc8_text(card.get("confirmed_time")),
        "标的 BTC ｜ 现价 " + _fmt_price_thousands(card.get("price"))
        + " ｜ 卡号 #" + cid,
        "",
        "◎ 结论",
        ...
        "",
        "◎ 发生了什么",
        _what_happened_sentence(window, reasoning),
        "",
    ]
    head, body = _why_section(conclusion, blocking, conflict)
    lines.extend(["◎ " + head, body, ""])
    lines.extend(["◎ 参与论证（按贡献）",
                  _participants_block(reasoning, blocking), ""])
    lines.extend(["◎ 置信链", _confidence_chain_line(decomp, blocking), ""])
    lines.append("全量结构化留档 → signal_review.jsonl #" + cid)
    return "\n".join(lines)


def _conclusion_sentence(conclusion):
    lean_cn = conclusion.get("lean_cn") or "方向中性"
    support = conclusion.get("support_label")
    if support in ("TRADE_SUPPORT_STRONG", "TRADE_SUPPORT_WEAK"):
        action = "可交易（下游侧建议：" + (conclusion.get("side_hint_cn") or "-") + "）"
    elif support == "WAIT_CONFIRMATION":
        action = "暂不交易、等待确认"
    else:
        action = "暂不交易"
    return "{0}、{1}，置信 {2}/100。".format(
        lean_cn, action, _fmt_int(conclusion.get("confidence")))


def _what_happened_sentence(window, reasoning):
    if window.get("is_active"):
        win = "DIE+Anchor 时序窗口刚确认修复"
    else:
        win = "DIE+Anchor 窗口未开（方向仅预热）"
    dirn = window.get("episode_direction")
    if dirn:
        win += "（位移{0}、峰{1}、合并{2}）".format(
            _dir_cn(dirn), _fmt_num(window.get("peak_m_die"), 2),
            _fmt_int(window.get("event_count_merged")))
    ev = [e for e in (reasoning.get("evidence") or [])
          if (e.get("eff_weight") or 0) > 0]
    score = reasoning.get("edb_score")
    side = "看空" if (score or 0) < 0 else ("看多" if (score or 0) > 0 else "方向中性")
    lead = ev[:3]
    lead_txt = "、".join("{0}{1}({2})".format(
        e.get("gloss_cn"), e.get("lean_cn"), _fmt_signed(e.get("vote"), 2))
        for e in lead) or "无有效方向证据"
    agr = reasoning.get("agreement")
    互证 = "且互相印证" if (agr is not None and agr >= 0.80) else "（存在分歧）"
    return ("{0}。方向层 {1} 项因子参与、整体{2}{3}：{4} → 合成 "
            "EDB={5}、一致度{6}。").format(
        win, len(ev), side, 互证, lead_txt,
        _fmt_signed(score, 2), _fmt_pct(agr))


def _why_section(conclusion, blocking, conflict):
    hard = blocking.get("hard_veto")
    if hard:
        body = ("{0}：{1}，按风控规则当轮置信直接归零、不放行任何方向。"
                "这不是因子打架，是该门一票否决。").format(
                    hard.get("zh"), hard.get("evidence"))
        return "为何不交易", body
    support = conclusion.get("support_label")
    if support in ("TRADE_SUPPORT_STRONG", "TRADE_SUPPORT_WEAK"):
        body = "证据收敛达档（{0}）：可交易方向 {1}；下游侧建议 {2}。".format(
            conclusion.get("support_cn"), conclusion.get("lean_cn"),
            conclusion.get("side_hint_cn"))
        return "为何可交易", body
    gates = blocking.get("soft_gates") or []
    body = "；".join(g.get("zh") for g in gates) or "证据未达可交易档。"
    dissent = conflict.get("dissent") or []
    if conflict.get("level") in ("MATERIAL", "SEVERE") and dissent:
        body += "；存在对立项（{0}），证据分歧偏大。".format(
            "、".join(d.get("gloss_cn") for d in dissent))
    return "为何还在等", body


def _participants_block(reasoning, blocking):
    ev = [e for e in (reasoning.get("evidence") or [])
          if (e.get("eff_weight") or 0) > 0]
    if not ev:
        return "（本轮无有效方向证据参与）"
    parts = []
    for e in ev:
        tag = "" if e.get("aligned") else "·对立"
        parts.append("{0} {1}{2}·{3}".format(
            e.get("key"), _fmt_pct_int(e.get("contribution_pct")), tag,
            e.get("gloss_cn")))
    note = _excluded_note(reasoning, blocking)
    line = "　".join(parts)
    if note:
        line += "\n（" + note + "）"
    return line
...
def _excluded_note(reasoning, blocking):
    ...
    for key in excluded:
        if key == "MACRO" and veto_reason == "MACRO_BLOCKING":
            ...
            notes.append("{0} 未计入(权重0/数据不足/区制不给票)".format(
                evidence_gloss_cn(key)))
    return "；".join(notes)


def _confidence_chain_line(decomp, blocking):
    base = "强度{0} × 一致{1} × 覆盖{2} × Gamma门{3} = {4}".format(
        ...
        _fmt_int(decomp.get("conf_pre_veto")))
    tail = " → " + _fmt_int(decomp.get("confidence_final"))
    if blocking.get("hard_veto"):
        tail = " →(硬阻断) " + _fmt_int(decomp.get("confidence_final"))
    return base + tail
...
def _gex_info_table(info):
    ...
    if not info or info.get("quality") == "MISSING":
        return {
            ...
            "rows": [["status", "gexmonitorapi", "未接入/缺失",
                      "状态 " + _fmt((info or {}).get("data_state") or "missing"),
