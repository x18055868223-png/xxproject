...
CONFIG = {
    ...
    # source of truth, and FMZ pushes only a <=160-char single-line brief
    # (render_push_brief). Static web reads JSONL-derived JSON (frontend pending).
    ...
    # Push body format. True (default) = COMPACT, front-loaded ~6-line body that
    # fits FMZ's push length cap (the full 4-layer text gets truncated ~190 chars
    # and newlines flatten). False = full 4-layer text (only for channels that
    # deliver long bodies). Full structured detail is always in signal_review.jsonl.
    "signal_review_push_compact": True,
    ...
    # v1.0 audit standard: signal_review.jsonl holds the full machine record;
    # FMZ push is a <=160-char single-line brief that ends with an audit pointer.
    # Set this to the static audit site base (e.g. "https://audit.example.com")
    # to emit "审计:<base>/c/<id>"; empty -> the brief ends "详见FMZ Log #<id>".
...
def build_audit_record(card, config=None):
    ...
    record = {
        ...
        "market_context": {
            "runtime_mode": ("OBSERVE" if config.get("read_only_demo", True)
                             else "LIVE"),
        ...
        "conflict": {
            ...
            "dissent": [{"key": d.get("key"),
                         "vote": safe_float(d.get("vote")),
                         "effective_weight": safe_float(d.get("weight"))}
                        for d in (conflict.get("dissent") or [])],
        ...
        "delivery": {
            ...
            "local_card_json": _local_card_path(full_id, ms),
        },
    }
...
def render_push_brief(card, config=None):
    """FMZ ` @` push: a single-line brief (<=160 chars) — alert + navigation only,
    ...
    return ("【信号】" + str(config.get("asset", "BTC")) + " #" + short
            ...
            + " 冲突" + _fmt_pct(conflict.get("ratio"))
            + " 同" + same + " 反" + opp + " " + tail)
...
def _audit_quality(card):
    cross = card.get("factor_cross_section") or {}
    degraded = []
    for name, node in (("skew", cross.get("skew")),
                       ("gamma_regime", cross.get("gamma_regime")),
                       ("macro_pressure", cross.get("macro_pressure")),
                       ("gex_info", cross.get("gex_info"))):
        state = (node or {}).get("data_state") or (node or {}).get("data_status")
        if state in ("STALE", "MISSING", "DEGRADED", "partial", "cached"):
            degraded.append({"source": name, "state": state})
    return {
        "overall": "OK" if not degraded else "DEGRADED",
        "all_required_sources_ready": not degraded,
        "missing_fields": [],
...
def _audit_display_layers(card):
    ...
    return {
        "background": {
            "title_cn": "背景层",
            "summary_cn": ("窗口" + ("已确认修复" if window.get("is_active")
                                   else "未开")
                           + "；TMV blend " + _fmt_signed(tmvf.get("tmv_blend"), 2)
                           + "，Macro " + str(macro.get("macro_regime") or "-")
                           + "。"),
...
def _audit_blocking(blocking):
    ...
    return {
        ...
        "soft_gates": [{"gate": g.get("gate"), "reason_cn": g.get("zh")}
                       for g in (blocking.get("soft_gates") or [])],
        "unblock_conditions_cn": ([blocking.get("unblock_hint_cn")]
                                  if blocking.get("unblock_hint_cn") else []),
    }


def _audit_reasoning(reasoning, decomp, cal):
    evidence = []
    for e in reasoning.get("evidence") or []:
        key = e.get("key")
        evidence.append({
            "key": key,
            "gloss_cn": e.get("gloss_cn") or evidence_gloss_cn(key),
            "vote": safe_float(e.get("vote")),
            "weight": safe_float(e.get("weight")),
            "effective_weight": safe_float(e.get("eff_weight")),
            "information": safe_float(e.get("info")),
            "contribution_pct": safe_float(e.get("contribution_pct")),
            "aligned": bool(e.get("aligned")),
    ...
    return {
        ...
        "agreement": safe_float(reasoning.get("agreement")),
        "coverage": safe_float(reasoning.get("coverage")),
        "confidence_calibration": cal,
        "confidence_decomposition": {
        ...
        "evidence": evidence,
        "participants": reasoning.get("participants") or [],
    }
...
# --------------------------------------------------------------------------
# push 综述 — COMPACT default (fits FMZ's push length cap) + full 4-layer option
# --------------------------------------------------------------------------

def render_review_card_push(card, config=None):
    """Push body for one signal event. DEFAULT COMPACT: the FMZ app/email push
    has a practical length cap and flattens newlines, so the full 4-layer card
    is truncated mid-way (observed cut ~190 chars). The compact form front-loads
    the decision (it survives even a hard cut), reads fine when newlines collapse
    to spaces, and points to signal_review.jsonl for the full structured detail
    (which is always persisted in full). Set config.signal_review_push_compact=
    False to emit the full 4-layer text (only if the channel delivers long
    bodies). Single string; fmz_push() appends ' @'. Read-only."""
    config = config or CONFIG
    card = card or {}
    if config.get("signal_review_push_compact", True):
        return _render_push_compact(card)
    return _render_push_full(card)


def _render_push_compact(card):
    conclusion = card.get("conclusion") or {}
    reasoning = card.get("reasoning") or {}
    conflict = card.get("conflict") or {}
    blocking = card.get("blocking") or {}
    ...
    cal_tag = " 未校准" if cal != "CALIBRATED" else ""
    lines = [
        # L1 header — asset / card / time / price
        "【信号审计v1.2】BTC #" + cid + " " + utc8_text(card.get("confirmed_time"))
        + " 现价" + _fmt_price_thousands(card.get("price")),
        # L2 decision — front-loaded so it survives even a hard truncation
        (conclusion.get("lean_cn") or "方向中性")
        ...
        + " 一致" + _fmt_pct(reasoning.get("agreement"))
        + " 覆盖" + _fmt_pct(reasoning.get("coverage")),
        # L3 evidence — top same / opposite votes + conflict
        "同向 " + _compact_votes(reasoning, True)
        + " ｜反向 " + _compact_votes(reasoning, False)
        + " ｜冲突" + str(conflict.get("level") or "-")
        + _fmt_pct(conflict.get("ratio")),
        # L4 gate / block
        _compact_gate_line(card),
        # L5 background — TMV + Macro
        "背景 " + _compact_background(cross),
    ]
    foot = "详情→signal_review.jsonl #" + cid
    if cal != "CALIBRATED":
        foot += " ｜置信未校准·非真实胜率"
    lines.append(foot)
    return "\n".join(lines)
...
def _compact_votes(reasoning, aligned):
    ev = [e for e in (reasoning.get("evidence") or [])
          if (e.get("eff_weight") or 0) > 0 and bool(e.get("aligned")) == aligned]
    if not ev:
        return "无"
    return "、".join(str(e.get("key")) + _fmt_signed(e.get("vote"), 2)
                    for e in ev[:3])


def _compact_gate_line(card):
    conclusion = card.get("conclusion") or {}
    blocking = card.get("blocking") or {}
    hard = blocking.get("hard_veto")
    if hard:
        return ("阻断 " + str(hard.get("zh") or "硬阻断")
                + "(" + str(hard.get("evidence") or "-") + ")·置信归零")
    if not blocking.get("has_block"):
        return (_gamma_role_cn(card) + " ｜放行·侧 "
                + str(conclusion.get("side_hint_cn") or "-"))
    gates = blocking.get("soft_gates") or []
    gate = (gates[0].get("zh") if gates else None) or "等待确认"
    return (_gamma_role_cn(card) + " ｜" + gate
            + "·解除 " + str(blocking.get("unblock_hint_cn") or "-"))


def _compact_background(cross):
    tmvf = cross.get("tmvf") or {}
    macro = cross.get("macro_pressure") or {}
    t24 = _fmt_signed((tmvf.get("tmvf_24h") or {}).get("tmv_final"), 2)
    t48 = _fmt_signed((tmvf.get("tmvf_48h") or {}).get("tmv_final"), 2)
    return ("TMV" + _fmt_signed(tmvf.get("tmv_blend"), 2)
            + "(24h" + t24 + "/48h" + t48 + ") Macro"
            + _fmt_signed(macro.get("macro_score"), 2) + " "
            + str(macro.get("macro_regime") or "-"))


def _render_push_full(card):
    """Full 4-layer text (header + 背景/修正/论证/冲突 + replay index). Only for
    channels that deliver long push bodies (config.signal_review_push_compact=
    False); the default compact form is what survives FMZ's push length cap."""
    card = card or {}
    cid = str(card.get("card_id"))
...
class DemoRuntime:
    ...
    def _emit_signal_review_card(self):
        ...
        # FMZ push gets only the <=160-char brief (full chain stays in JSONL).
    ...
    def _emit_push_self_test(self):
        ...
        try:
            ...
            body = ("【推送自检·非真实信号】"
                    + render_push_brief(card, self.config))
        except Exception as exc:
