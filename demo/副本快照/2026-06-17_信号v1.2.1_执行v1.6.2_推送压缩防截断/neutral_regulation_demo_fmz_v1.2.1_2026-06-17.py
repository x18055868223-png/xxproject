...
CONFIG = {
    ...
    # truncated mid-way, so the push now defaults to a COMPACT front-loaded body
    # (signal_review_push_compact=True); full 4-layer text behind the flag; full
    # structured detail always in signal_review.jsonl. Render-only; schema stays
    # v1.0.0 (no new payload field, SignalEvidencePackage wire contract unchanged).
    "demo_version": "1.2.1",
...
class DemoRuntime:
    ...
    def _emit_signal_review_card(self):
        ...
        if self.config.get("signal_review_push_enabled", False):
            try:
                fmz_push(render_review_card_push(card, self.config))
    ...
    def _emit_push_self_test(self):
        ...
        try:
            ...
            body = ("【推送自检·非真实信号·仅验证链路样式】\n"
                    + render_review_card_push(card, self.config))
        except Exception as exc:
