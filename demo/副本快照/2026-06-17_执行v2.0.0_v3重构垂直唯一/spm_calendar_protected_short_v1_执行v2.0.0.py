...
# Deribit S:PM 垂直信用价差卖方执行链 v2.0.0（FMZ 单文件；单一 run_cycle 主链 + 交互控制台 + 对冲生命周期）
...
def exec_hedge_step(instrument, side, amount, reduce_only, allow_live, label="hedge"):
    """对冲一步：HEDGE_OPEN/INCREASE 非 reduce_only；HEDGE_REDUCE/UNWIND 强制 reduce_only。
    allow_live=False → 仅意图(dry)，不下单。返回 {filled, dry, reduce_only, reason}。"""
    if not side or not amount or amount <= 0:
        return {"filled": 0.0, "dry": (not allow_live), "reason": "NO_OP"}
    if not allow_live:
        return {"filled": 0.0, "dry": True, "side": side, "amount": amount,
                "reduce_only": reduce_only, "reason": "HEDGE_DRYRUN"}
    ...
    order = _extract_order(resp)
    if order is None:
        return {"filled": 0.0, "dry": False, "reduce_only": reduce_only, "reason": "HEDGE_ORDER_FAILED"}
    oid = order.get("order_id")
    st = _extract_order(dbt_get_order_state(oid)) or order
    return {"filled": st.get("filled_amount") or 0.0, "dry": False,
...
设计稿 §10 + 补充意见 P0-5：
  - 固定工具 BTC-PERPETUAL @ DERIBIT（删 UNBOUND / Binance 场所选择）；
...
def hedge_target_contracts(remaining_short_qty, structure_delta, reduction_ratio,
                           ...
                           option_structure_state="OPEN"):
    """对冲目标(合约张数)。硬不变量：短腿归零 或 结构 CLOSED/SETTLED → 0（不等保护腿出售）。
    BTC delta 敞口 → USD 合约：raw = |rem_qty·delta·ratio|·spot / contract_size，取整到 min_trade。"""
    if str(option_structure_state).upper() in ("CLOSED", "SETTLED"):
        return 0.0
    if not remaining_short_qty or remaining_short_qty <= _EPS:
        return 0.0
    if not (_is_num(structure_delta) and _is_num(spot) and _is_num(contract_size)) or contract_size <= 0:
        return 0.0
...
def _evaluate_hedge(snap):
    """对冲决策：perp 真实持仓 + 目标(随剩余短腿敞口) + open/reduce 动作 + 孤儿。默认不真实下单。
    每轮从 BTC-PERPETUAL instrument metadata 读 contract_size/min_trade。"""
    ...
    meta = dbt_get_instrument(HEDGE_INSTRUMENT) or {}
    ...
    return {"perp_qty": perp_qty, "target": target,
            ...
            "side": hedge_side((snap or {}).get("side")), "instrument": HEDGE_INSTRUMENT}
...
def manage_cycle(now_ms):
    ...
    if h_act != "HEDGE_HOLD":
        ...
        if gate_decision(h_gate_act, ALLOW_ENTRY_TRADING, ALLOW_EXIT_TRADING, ALLOW_HEDGE_TRADING,
            ...
            exec_hedge_step(hedge["instrument"], hedge["side"], hedge["action"]["delta_contracts"],
...
def run_cycle(now_ms=None):
    ...
    if manage_result:
        ...
        if _h:
            ctx["hedge_state"] = "%s 目标%.0f 当前%.0f %s%s" % (
                _h.get("side") or "—", _h.get("target") or 0.0, _h.get("perp_qty") or 0.0,
                _h["action"]["action"], "·孤儿" if _h.get("orphan") else "")
