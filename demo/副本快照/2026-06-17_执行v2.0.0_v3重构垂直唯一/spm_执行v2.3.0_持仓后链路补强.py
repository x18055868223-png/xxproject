...
# Deribit S:PM 垂直信用价差卖方执行链 v2.3.0（FMZ 单文件；单一 run_cycle 主链 + 交互控制台 + 对冲生命周期）
...
def manage_cycle(now_ms):
    ...
    **退出活动期禁新增对冲敞口**（只许 reduce/unwind）。退出/对冲/回收真实下单均受各自门控，默认空跑。"""
    ...
    exit_decision = exit_campaign_decision(authorized, tp["qualified"], rem_short,
    ...
    if exit_active and hedge["action"]["action"] in ("HEDGE_OPEN", "HEDGE_INCREASE"):
        hedge["action"] = {"action": "HEDGE_HOLD", "reduce_only": False, "delta_contracts": 0.0}  # P0③退出期禁新增对冲
    ...
    hedge_exec = (hedge["action"]["action"] != "HEDGE_HOLD"
                  and gate_decision(h_gate_act, ALLOW_ENTRY_TRADING, ALLOW_EXIT_TRADING,
    ...
    arb = unified_action_arbiter({
        ...
        "in_flight_order": False, "exit_preferred": False, "hedge_ready": False,  # 风险严重度 hook(P1)
        "take_profit_ready": tp["qualified"],
        "exit_authorized": authorized,
        "exit_executable": bool(exit_decision["can_order"] and exit_gate),
    ...
    if executable in ("TAKE_PROFIT_READY", "EXIT_PREFERRED") and rem_short > 1e-12 \
            and exit_decision["can_order"] and exit_gate:
        step = exec_exit_buyback_step(snap.get("short_instrument"), rem_short,
