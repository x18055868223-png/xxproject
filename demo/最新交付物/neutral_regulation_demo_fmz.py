...
CONFIG = {
    ...
    # 冲突 + replay index) + signal_review_push_test self-test flag.
    # v1.2.1 (2026-06-17): the 4-layer body exceeded FMZ's push length cap and was
    # truncated mid-way, so the push defaulted to a compact front-loaded body.
    # v1.3.0 (2026-06-18): even the compact body (~324c) was truncated -> adopt the
    # audit-JSON standard (docs/信号审计JSON推送与静态Web标准_v1.0.md): the full v1.0
    ...
    # 下方“模型内部默认区”是算法阈值和降级参数，除非做版本化校准，否则不建议临场修改。
    # ============================================================
    # 标的配置: 三个市场源必须指向同一资产，否则审计卡会出现跨源错配。
    "asset": "BTC",
    "spot_symbol": "BTCUSDT",
    "futures_symbol": "BTCUSDT",
    "deribit_currency": "BTC",

    # 运行模式: 当前交付物仍是只读观察层，不下单、不持仓、不接执行层。
    "read_only_demo": True,
    ...
    # FMZ 可视化与日志: 控制启动摘要、轮询摘要、状态面板和图表显示密度。
    ...
    # HTTP 通用配置: 所有公开行情/GEX/Macro 请求共用，轻量服务器建议保持短超时。
    "http_timeout_sec": 5,
    "http_retries": 2,
    "http_retry_delays": [0.6, 1.2],

    # Binance 行情源: 公开行情读取，不需要账户密钥。
    ...
    # GEX 原始公共源: gexmonitor 最新值的兼容读取层。
    ...
    # 信号审计输出: 全量 JSONL 永远本地落盘，FMZ 只发短推。
    # push_test 只用于部署自检，正常等待真信号时保持 False。
    # audit_static_base_url 暂留空，因为当前静态前端还未启用 /c/<short_id> 深链。
    ...
    # ============================================================
    # 模型内部默认区: 信号阈值、证据权重、降级规则和缓存策略。
    # 这些参数是版本交付物的一部分，修改前应另起版本并留下回放/审计证据。
    # ============================================================
...
USER_CONFIG_KEYS = (
    ...
    "read_only_demo",
    "live_fetch_enabled",
    "offline_fixture_enabled",
    "max_main_loops",
    "loop_sleep_ms",
    "continue_on_tick_error",
    "error_sleep_ms",
    "startup_log_enabled",
    "tick_summary_log_enabled",
    "tick_summary_log_every",
    "state_change_log_enabled",
    "log_status_enabled",
    "status_max_lines",
    "chart_enabled",
    "chart_reset_on_start",
    "http_timeout_sec",
    "http_retries",
    "http_retry_delays",
    "binance_spot_base_url",
    "binance_futures_base_url",
    "binance_spot_agg_trades_path",
    "binance_futures_agg_trades_path",
    "agg_trades_limit",
    "spot_depth_limit",
    "macro_yahoo_base_url",
    "macro_refresh_sec",
    "macro_range",
    "macro_interval",
    "macro_cache_file",
    ...
    "deribit_base_url",
    "deribit_index_name",
    "deribit_instruments_refresh_sec",
    "deribit_min_expiry_hours",
    "deribit_max_expiry_days",
    "strategy_expiry_targets_hours",
    "gex_base_url",
    "gex_exchange",
    "gex_lite",
    "gex_info_enabled",
    "gex_info_base_url",
    "gex_info_token",
    "gex_info_refresh_sec",
    "gex_info_cache_file",
    "gex_info_cache_max_age_ms",
...
USER_CONFIG_DOC_CN = {
    "asset": "主审计资产简称，展示、日志和审计卡标题使用。",
    "spot_symbol": "Binance 现货交易对，必须与 asset 对应。",
    "futures_symbol": "Binance U 本位合约交易对，用于主动成交流和资金费率读取。",
    "deribit_currency": "Deribit 期权币种，用于 SRD/GGR 期权证据。",
    "read_only_demo": "只读观察保护开关；当前交付物应保持 True，不下单。",
    "live_fetch_enabled": "是否读取公开实时行情；False 时走离线或样例路径。",
    "offline_fixture_enabled": "是否启用内置样例数据，只用于本地调试。",
    "max_main_loops": "主循环次数，0 表示在 FMZ 上持续运行。",
    "loop_sleep_ms": "每轮主循环间隔毫秒，控制行情刷新节奏。",
    "continue_on_tick_error": "单轮异常后是否继续下一轮，实盘观察建议保持 True。",
    "error_sleep_ms": "单轮异常后的等待毫秒数，避免错误时高频重试。",
    "startup_log_enabled": "是否打印启动摘要，便于确认部署模式。",
    "tick_summary_log_enabled": "是否打印每轮观察摘要。",
    "tick_summary_log_every": "观察摘要打印间隔，1 表示每轮打印。",
    "state_change_log_enabled": "是否只在结论状态变化时额外打印状态变化。",
    "log_status_enabled": "是否渲染 FMZ 状态栏表格。",
    "status_max_lines": "状态栏最大行数，避免 FMZ 页面过长。",
    "chart_enabled": "是否更新 FMZ 图表。",
    "chart_reset_on_start": "启动时是否重置 FMZ 图表。",
    "http_timeout_sec": "公开接口请求超时秒数。",
    "http_retries": "公开接口失败重试次数。",
    "http_retry_delays": "公开接口重试等待秒数列表。",
    "binance_spot_base_url": "Binance 现货公开 API 根地址。",
    "binance_futures_base_url": "Binance 合约公开 API 根地址。",
    "binance_spot_agg_trades_path": "Binance 现货主动成交接口路径。",
    "binance_futures_agg_trades_path": "Binance 合约主动成交接口路径。",
    "agg_trades_limit": "主动成交单次读取条数上限。",
    "spot_depth_limit": "现货深度读取档位数。",
    "macro_yahoo_base_url": "Yahoo 宏观代理公开接口根地址。",
    "macro_refresh_sec": "宏观证据刷新间隔秒数。",
    "macro_range": "宏观证据读取窗口。",
    "macro_interval": "宏观证据 K 线粒度。",
    "macro_cache_file": "宏观证据本地缓存文件。",
    "macro_symbols": "宏观证据主符号映射。",
    "macro_symbol_candidates": "宏观证据备用符号列表。",
    "deribit_base_url": "Deribit 公开 API 根地址。",
    "deribit_index_name": "Deribit 指数名，需匹配当前资产。",
    "deribit_instruments_refresh_sec": "Deribit 期权列表刷新间隔秒数。",
    "deribit_min_expiry_hours": "纳入审计的最短期权到期小时数。",
    "deribit_max_expiry_days": "纳入审计的最长期权到期天数。",
    "strategy_expiry_targets_hours": "审计关注的期权到期目标小时列表。",
    "gex_base_url": "旧 GEX 公共源地址，作为兼容读取层。",
    "gex_exchange": "旧 GEX 公共源交易所过滤参数。",
    ...
    "signal_review_enabled": "是否把真信号审计卡写入 signal_review.jsonl。",
...
decomposition) into one card, then renders it three ways:
  ...
  - <=140-char FMZ alert brief -> render_push_brief
...
_SESSION_PREMISE_CONTEXTS = {
    "POST_US_DEADZONE": {
        ...
        "rationale_cn": (
            ...
            "但薄盘尾部插针属于均值口径难覆盖的尾部风险，本版本保持中性保守，"
            "不据此升耐久，也不改写 confidence。"),
    },
    "ASIA_MORNING": {
    ...
    "LONDON_EARLY": {
        ...
        "catalyst_exposure": "PRE_US_AHEAD",
        "adjustment_direction": "NEUTRAL",
        "evidence_level": "TENTATIVE",
        "axis": "EU_LIQUIDITY_US_AHEAD",
        "operator_hint_cn": "偏耐久但未确认，保持中性观察。",
        "rationale_cn": (
            "15:00-18:00 UTC+8 欧洲早盘补流动性，60m 代理显示 -1.37pp 偏耐久但不稳；"
            "维持中性观察，不据此改写 confidence。"),
    },
    "PRE_US_TRAPDOOR": {
    ...
    "US_OPEN_TURBULENCE": {
        ...
        "rationale_cn": (
            "21:30-23:00 UTC+8 为纽约开盘湍流阶段，60m 代理显示 +1.49pp 偏脆但不稳；"
            "本版本保持中性保守，避免过早升高前提耐久度，不改写 confidence。"),
    },
    "US_DEEP_POST_CATALYST": {
        ...
        "rationale_cn": (
            "23:00-04:00 UTC+8 属美盘深流动性/催化剂已消化窗口；三年 K 线代理显示"
            "复合重写率 -1.49pp，方向与理论一致但仍属暂定，因此只作为中等幅度提高"
            "前提耐久度的人工提示，不改写 confidence。"),
    },
}
...
def classify_signal_session_context(confirmed_time_ms, config=None):
    ...
    if safe_float(confirmed_time_ms) is None:
        return {
            ...
            "axis": "MISSING_CONFIRMED_TIME",
            "rationale_code": "MISSING_CONFIRMED_TIME",
            "rationale_cn": "确认时间缺失，时区前提无法判定；观察层按低前提耐久度保守展示。",
            "operator_hint_cn": "补齐确认时间后再判定时区前提耐久度。",
            "transition": {"active": False, "policy": "DISPLAY_ONLY_NO_CONFIDENCE_CHANGE"},
            ...
            "event_blackout": {"active": False},
            "calibration_state": "MARKET_PRIOR_VALIDATED_NOT_SIGNAL_CALIBRATED",
            "confidence_policy": "DO_NOT_MULTIPLY_CONFIDENCE",
            "confidence_multiplier": 1.0,
            "validation_basis": dict(_SESSION_VALIDATION_BASIS),
    ...
    template = dict(_SESSION_PREMISE_CONTEXTS.get(code) or {})
    if not template:
        liquidity_depth, catalyst_exposure = _session_zone_props(base_zone, code)
        template = {
            "clock_window": None,
            "start_min": None,
            "end_min": None,
            "backtest_delta_pp": None,
            "theory_zone": base_zone,
            "base_zone": base_zone,
            "effective_zone": "LOW",
            "display_label": code,
            "premise_durability": "LOW",
            ...
            "adjustment_direction": "DECREASE",
            "evidence_level": "EVENT" if event_blackout else "UNKNOWN",
            "axis": code,
            "operator_hint_cn": "事件窗口仅作为观察层提示，不改写 confidence。",
            "rationale_cn": _SESSION_RATIONALE_CN.get(code, code),
        }

    transition = {
        "active": False,
        "boundary": template.get("clock_window", "").split("-")[0] or None,
        "minutes_from_boundary": None,
        "policy": "DISPLAY_ONLY_NO_CONFIDENCE_CHANGE",
    }
    weekend = times["sh"].weekday() >= 5
    weekend_adjustment = {"applied": False}
    event_context = event_blackout or {"active": False}

    return {
        "schema": "SignalSessionPremiseDurabilityContext@1.0.0",
        "schema_name": "SignalSessionPremiseDurabilityContext",
        "schema_version": "1.0.0",
        "clock_window": template.get("clock_window"),
        "start_min": template.get("start_min"),
        "end_min": template.get("end_min"),
        "backtest_delta_pp": template.get("backtest_delta_pp"),
        "theory_zone": template.get("theory_zone", base_zone),
        "base_zone": template.get("base_zone", base_zone),
        "effective_zone": template.get("effective_zone"),
        "display_label": template.get("display_label"),
        "premise_durability": template.get("premise_durability"),
        "liquidity_depth": template.get("liquidity_depth"),
        "catalyst_exposure": template.get("catalyst_exposure"),
        "adjustment_direction": template.get("adjustment_direction"),
        "evidence_level": template.get("evidence_level"),
        "axis": template.get("axis"),
        "rationale_code": code,
        "rationale_cn": template.get("rationale_cn"),
        "operator_hint_cn": template.get("operator_hint_cn"),
...
# --------------------------------------------------------------------------
# v1.0 audit record (full machine JSON for signal_review.jsonl) + brief push
# standard: docs/信号审计JSON推送与静态Web标准_v1.0.md
# --------------------------------------------------------------------------
...
def build_audit_record(card, config=None):
    ...
    Read-only RESHAPE: no recompute, no change to direction/confidence/gating.
    Missing data stays null (never 0); degraded sources are listed in `quality`.
    Spec: docs/信号审计JSON推送与静态Web标准_v1.0.md."""
    config = config or CONFIG
    card = card or {}
    conclusion = card.get("conclusion") or {}
    ...
    ms = card.get("confirmed_time")
    short = str(card.get("card_id"))
    symbol = str(config.get("asset", "BTC"))
    episode = card.get("episode_id")
    ...
    record = {
        ...
        "identity": {
            ...
            "strategy_version": config.get("demo_version"),
        ...
        "market_context": {
            ...
            "price_source": _source_ref_for("price"),
            "quote_currency": config.get("quote_currency", "USDT"),
            "bar_interval": config.get("tmvf_kline_interval", "1h"),
        },
        "decision": {
            "lean": conclusion.get("lean"),
        ...
        "signal_window": {
            ...
            "episode_direction": window.get("episode_direction"),
            "peak_m_die": safe_float(window.get("peak_m_die")),
            "event_count_merged": window.get("event_count_merged"),
            "anchor_score": safe_float(window.get("anchor_score")),
            "anchor_normalized_deviation": safe_float(window.get("anchor_nd")),
            "interpretation_cn": (
                "时序窗口已确认修复，但窗口只确定审计时点，不单独决定交易方向。"
                if window.get("is_active") else "时序窗口未开，方向仅作观察预热。"),
            "session_context": session_context,
        },
        "decision_matrix": _audit_decision_matrix(
            conclusion, blocking, conflict, session_context),
        ...
        "conflict": {
            ...
            "aligned_keys": conflict.get("aligned_keys") or [],
            "dissent_keys": [d.get("key") for d in (conflict.get("dissent") or [])
                             if d.get("key")],
            "dominant_conflict": _dominant_conflict(conflict),
            "explanation_cn": conflict.get("explanation_cn"),
        },
        "factor_cross_section": _audit_cross_section(cross, card),
        "delivery": {
            "fmz_push_summary": _safe_brief(card, config),
            "fmz_log_ref": "signal_review card_id=" + full_id,
            "static_web_url": _static_web_url(full_id, config),
            "local_jsonl": "demo/logs/signal_review.jsonl",
...
def _audit_decision_matrix(conclusion, blocking, conflict, session_context):
    support = conclusion.get("support_label")
    has_block = bool((blocking or {}).get("has_block"))
    hard_veto = (blocking or {}).get("hard_veto") or {}
    conflict_ratio = safe_float((conflict or {}).get("ratio"))
    if support in ("TRADE_SUPPORT_STRONG", "TRADE_SUPPORT_WEAK") and not has_block:
        decision_state = "APPROVABLE"
    elif hard_veto:
    ...
    return {
        ...
        "temporal_durability": (session_context or {}).get("premise_durability"),
        "audit_dissent": audit_dissent,
        "execution_allowed": bool((blocking or {}).get("execution_allowed")),
        "model_trade_support": support in ("TRADE_SUPPORT_STRONG",
                                           "TRADE_SUPPORT_WEAK"),
        "reason_codes": _audit_decision_reason_codes(support, blocking),
    }


def _audit_decision_reason_codes(support, blocking):
    codes = []
    hard_veto = (blocking or {}).get("hard_veto") or {}
    if hard_veto.get("veto_reason"):
        codes.append(str(hard_veto.get("veto_reason")))
    for gate in (blocking or {}).get("soft_gates") or []:
        if isinstance(gate, dict) and gate.get("reason_code"):
            codes.append(str(gate.get("reason_code")))
    if support:
        codes.append("SUPPORT_LABEL_" + str(support))
    return codes


def render_push_brief(card, config=None):
    """FMZ ` @` push: a single-line brief (target <=140 chars) — alert + navigation only,
    decision front-loaded, audit pointer last. The full evidence chain lives in
    signal_review.jsonl / the static page. Spec §6. Never changes signal output."""
    config = config or CONFIG
    card = card or {}
    conclusion = card.get("conclusion") or {}
    reasoning = card.get("reasoning") or {}
    ...
    aligned = [e for e in ev if e.get("aligned")][:2]
    dissent = [e for e in ev
               if not e.get("aligned") and (safe_float(e.get("vote")) or 0) != 0][:2]
    same = ",".join(_brief_key(e.get("key")) + _fmt_signed(e.get("vote"), 2)
                    for e in aligned) or "无"
...
def _safe_brief(card, config):
    """Embed the brief in the record without letting a summary-render error break
    the record build (the JSONL record is the source of truth and must persist)."""
    try:
        return render_push_brief(card, config)
    except Exception:
        return None
...
def _self_test_push_body(card, config, wrote_json):
    ...
    banner = "【推送自检·非真实信号】"
    body = banner + render_push_brief(card, config) + status
    if len(body) <= 140:
        return body
...
def _brief_key(key):
    return str(key or "-").split("_")[0]


def _brief_keys(items):
    return ",".join(_brief_key(e.get("key")) for e in items) or "无"


def _brief_pct(value):
    value = safe_float(value)
    if value is None:
        return "-"
...
def _brief_dir(lean_cn):
    return (str(lean_cn or "中性").replace("方向", "").replace("(预热)", "")
            or "中性")


def _brief_action(support):
    return {
        "TRADE_SUPPORT_STRONG": "可交易", "TRADE_SUPPORT_WEAK": "可交易",
        "WAIT_CONFIRMATION": "等待确认", "NO_TRADE_BLOCKED": "阻断",
        "NO_TRADE_AMBIGUOUS": "歧义",
    }.get(support, support or "-")


def _iso8601_utc8(ms):
    ms = safe_float(ms)
    if ms is None:
        return None
    tz = datetime.timezone(datetime.timedelta(hours=8))
    return datetime.datetime.fromtimestamp(ms / 1000.0, tz).strftime(
        "%Y-%m-%dT%H:%M:%S+08:00")


def _full_card_id(ms, symbol, episode, short):
    ms = safe_float(ms)
    if ms is None:
        stamp = "00000000T000000+0800"
    else:
        tz = datetime.timezone(datetime.timedelta(hours=8))
        stamp = datetime.datetime.fromtimestamp(ms / 1000.0, tz).strftime(
            "%Y%m%dT%H%M%S+0800")
    return "-".join([stamp, str(symbol), str(episode), str(short)])


def _local_card_path(full_id, ms):
    ms = safe_float(ms)
    if ms is None:
        ymd = "0000/00/00"
    else:
...
def _audit_provenance(config, event_time_ms=None):
    ...
    return {
        ...
        "component_versions": {
            "signal_review_builder": version,
            "schema": config.get("schema_version"),
            "edb": "embedded:" + version,
            "tmvf": "embedded:" + version,
            "neutral_repair": "embedded:" + version,
...
def _source_ref_for(key):
    return {
        "price": "BINANCE_SPOT",
        "neutral_repair": "INTERNAL_DIE_ANCHOR",
        "tmvf": "BINANCE_KLINE_AND_FUNDING",
        "micro_flow": "BINANCE_AGG_TRADES",
        "funding": "BINANCE_FUNDING_RATE",
        "macro_pressure": "YAHOO_FINANCE",
        "gamma_regime": "DERIBIT_OPTIONS",
        "gex_info": "GEX_MONITOR_API",
        "skew": "DERIBIT_OPTIONS",
    }.get(key, key)


def _audit_status(value, default="OK"):
...
def _audit_cross_section(cross, card):
    cross = cross or {}
    window = card.get("window") or {}
    anchor = dict(cross.get("anchor") or {})
    anchor.setdefault("score", safe_float(window.get("anchor_score")))
    anchor.setdefault("normalized_deviation", safe_float(
        window.get("anchor_nd") or anchor.get("normalized_deviation")))
    anchor.setdefault("source_ref", _source_ref_for("neutral_repair"))

    m_die = dict(cross.get("m_die") or {})
    if "value" not in m_die:
        m_die["value"] = safe_float(m_die.get("m_die"))
    m_die.setdefault("direction", window.get("episode_direction"))
    m_die.setdefault("source_ref", "BINANCE_1M_KLINE")

    nr_src = cross.get("neutral_repair") or {}
    nr = dict(nr_src)
    event_context = nr_src.get("event_context") or {}
    anchor_context = nr_src.get("anchor_context") or {}
    nr.setdefault("episode_id", card.get("episode_id"))
    nr.setdefault("episode_direction", window.get("episode_direction")
                  or event_context.get("episode_direction"))
    nr.setdefault("peak_m_die", safe_float(window.get("peak_m_die")
                  or event_context.get("peak_m_die")))
    nr.setdefault("event_count_merged", window.get("event_count_merged")
                  or event_context.get("event_count_merged"))
    nr.setdefault("anchor_score", safe_float(window.get("anchor_score")
                  or anchor_context.get("anchor_score")))
    nr.setdefault("anchor_normalized_deviation", safe_float(
        window.get("anchor_nd") or anchor_context.get("normalized_deviation")))

    tmvf = dict(cross.get("tmvf") or {})
    for key in ("tmvf_24h", "tmvf_48h"):
        child = dict(tmvf.get(key) or {})
        if "final" not in child:
            child["final"] = safe_float(child.get("tmv_final"))
        tmvf[key] = child
    tmvf.setdefault("source_ref", "BINANCE_1H_KLINE")

    micro = dict(cross.get("micro_flow") or {})
    if "slow_12h" not in micro:
        micro["slow_12h"] = {"data_ready": False,
                             "status": "NOT_EMITTED_BY_EDB"}
    micro.setdefault("source_ref", _source_ref_for("micro_flow"))
    micro.setdefault("data_status", _audit_status(micro.get("data_status"),
                                                  "OK" if micro else "MISSING"))

    funding = dict(cross.get("funding") or {})
    ...
    if "score" not in macro:
        macro["score"] = safe_float(macro.get("macro_score"))
    if "regime" not in macro:
        macro["regime"] = macro.get("macro_regime")
    if "data_confidence" not in macro:
        macro["data_confidence"] = safe_float(macro.get("macro_data_confidence"))
    macro["data_status"] = _audit_status(macro.get("data_status"),
                                         "OK" if macro else "MISSING")
    macro.setdefault("source_ref", _source_ref_for("macro_pressure"))

    gamma = dict(cross.get("gamma_regime") or {})
    if "net_gamma_notional_usd" not in gamma:
    ...
    skew = dict(cross.get("skew") or {})
    if "data_status" not in skew:
        skew["data_status"] = skew.get("data_state")
    skew.setdefault("source_ref", _source_ref_for("skew"))
...
def _audit_gex_info(raw):
    if isinstance(raw, dict) and raw:
        ...
        gex = {
            ...
            "data_status": "MISSING",
            "quality": "MISSING",
            "reason": "GEX_INFO_MISSING",
        }
    gex.setdefault("source_ref", _source_ref_for("gex_info"))
    status = _audit_status(gex.get("data_status")
                           or gex.get("data_state")
                           or gex.get("availability")
                           or gex.get("quality"),
                           "MISSING")
...
def _gex_wall_level(walls):
    if not walls:
        return None
    first = walls[0]
    if isinstance(first, dict):
        return safe_float(first.get("strike") or first.get("price")
                          or first.get("level"))
    return safe_float(first)


def _audit_quality(card):
    ...
    specs = (
        ...
        ("macro_pressure", True, cross.get("macro_pressure") or {}),
        ("gamma_regime", True, cross.get("gamma_regime") or {}),
        ("gex_info", False, cross.get("gex_info") or {}),
        ("skew", True, cross.get("skew") or {}),
    )
    sources, degraded, missing = {}, [], []
    for name, required, node in specs:
        status = _audit_status((node or {}).get("data_status")
        ...
        if required and (not node or status == "MISSING"):
            missing.append("factor_cross_section." + name)
        if status in ("STALE", "MISSING", "DEGRADED", "PARTIAL"):
            degraded.append({"source": name, "status": status,
                             "reason": reason or status})
    for path, value in (
            ("market_context.price", card.get("price")),
            ("decision.confidence", (card.get("conclusion") or {}).get("confidence")),
            ("reasoning.score.final", (card.get("reasoning") or {}).get("edb_score")),
            ("conflict.ratio", (card.get("conflict") or {}).get("ratio"))):
        if value is None:
            missing.append(path)
    blocking_required = [d for d in degraded if sources[d["source"]]["required"]]
    return {
        "overall": "OK" if not missing and not blocking_required else "DEGRADED",
        "all_required_sources_ready": not missing and not blocking_required,
        "missing_fields": missing,
        "degraded_sources": degraded,
        "optional_missing_sources": (
            ["factor_cross_section.gex_info"]
            if sources.get("gex_info", {}).get("status") == "MISSING" else []),
        "sources": sources,
    }


def _audit_display_layers(card):
    reasoning = card.get("reasoning") or {}
    conflict = card.get("conflict") or {}
    cross = _audit_cross_section(card.get("factor_cross_section") or {}, card)
    window = card.get("window") or {}
    ...
    decomp = reasoning.get("confidence_decomposition") or {}
    return {
        "headline": _build_headline(card),
        "background": {
            "title_cn": "背景层",
            "summary_cn": ("现价 " + _fmt_price_thousands(card.get("price"))
                           + "；窗口" + ("已确认" if window.get("is_active") else "未开")
                           + "；TMV blend " + _fmt_signed(tmvf.get("tmv_blend"), 2)
                           + "，Macro " + str(macro.get("regime") or "-") + "。"),
            "source_refs": ["market_context", "signal_window",
                            "factor_cross_section.tmvf",
                            "factor_cross_section.macro_pressure"],
        },
        "correction": {
            "title_cn": "修正层",
            "summary_cn": ("Gamma " + str(ggr.get("regime") or "-")
                           + " 置信乘子 " + _fmt_num(decomp.get("ggr_mult"), 2)
                           + "；Funding/Skew 见原始截面。"),
            "source_refs": ["blocking", "factor_cross_section.gamma_regime",
                            "factor_cross_section.funding",
                            "factor_cross_section.skew"],
        },
        "reasoning": {
            "title_cn": "论证层",
            "summary_cn": ("EDB " + _fmt_signed(reasoning.get("edb_score"), 2)
                           + "，证据强度 " + _fmt_int(_audit_strength_pct(decomp))
                           + "，置信 " + _fmt_int((card.get("conclusion") or {}).get(
                               "confidence")) + "。"),
            "source_refs": ["reasoning.score",
                            "reasoning.confidence_decomposition",
                            "reasoning.evidence"],
        },
        "conflict": {
            "title_cn": "冲突层",
            "summary_cn": conflict.get("explanation_cn") or "",
            "source_refs": ["conflict", "blocking"],
        },
        "operator_focus": _operator_focus(card),
    }
...
def _audit_blocking(blocking):
    ...
    for gate in (blocking.get("soft_gates") or []):
        soft.append({
            "gate": gate.get("gate"),
            "reason_code": gate.get("reason_code") or gate.get("gate"),
            "reason_cn": gate.get("zh") or gate.get("reason_cn"),
        })
    conditions = []
    if blocking.get("unblock_hint_cn"):
        conditions.append({
            "metric": "decision.confidence",
            "operator": ">=",
            "threshold": 50,
            "condition_cn": blocking.get("unblock_hint_cn"),
        })
    return {
...
def _audit_reasoning(reasoning, decomp, cal):
    ...
    abs_total = sum(abs(safe_float(row.get("weighted_contribution")) or 0.0)
                    for row in rows)
    for row in rows:
        weighted = safe_float(row.get("weighted_contribution")) or 0.0
        row["absolute_share_pct"] = (
            abs(weighted) / abs_total * 100.0 if abs_total > 1e-12 else 0.0)
    weighted_sum = sum(safe_float(row.get("weighted_contribution")) or 0.0
                       for row in rows)
    eff_sum = sum(safe_float(row.get("effective_weight")) or 0.0 for row in rows)
    agreement_value = safe_float(reasoning.get("agreement"))
    coverage_value = safe_float(reasoning.get("coverage"))
    return {
        "engine": "EDB",
        "engine_version": "0.5",
        "score": {
            "method": "sum(vote * effective_weight) / sum(effective_weight)",
            "weighted_vote_sum": weighted_sum,
            "effective_weight_sum": eff_sum,
            "raw": safe_float(reasoning.get("edb_score_raw")),
            "smoothing": {"applied": False},
            "final": safe_float(reasoning.get("edb_score")),
        },
        "agreement": {
            "method": "aligned_effective_weight / effective_weight_sum",
            "value": agreement_value,
        },
        "coverage": {
            "method": "informative_required_evidence / configured_required_evidence",
            "value": coverage_value,
        },
        "confidence_calibration": cal,
        "confidence_decomposition": {
            "formula": ("100 * strength * agreement_factor * coverage_factor"
                        " * ggr_multiplier"),
            "strength": safe_float(decomp.get("strength")),
            "strength_formula": "abs(score.final) / score_full",
            "score_full": safe_float(decomp.get("score_full")),
            "agreement_raw": agreement_value,
            "agreement_floor": safe_float(decomp.get("agreement_floor")),
            "agreement_factor": safe_float(decomp.get("agr_factor")),
            "coverage_raw": coverage_value,
            "coverage_floor": safe_float(decomp.get("coverage_floor")),
...
def _audit_evidence_row(row):
    key = row.get("key")
    vote = safe_float(row.get("vote"))
    configured = safe_float(row.get("weight"))
    effective = safe_float(row.get("eff_weight"))
    info = safe_float(row.get("info"))
    weighted = None if vote is None or effective is None else vote * effective
    reliability = (effective / configured if configured and effective is not None
                   else (1.0 if configured == effective and configured is not None else None))
    return {
        "key": key,
        ...
        "net_contribution_pct": safe_float(row.get("contribution_pct")),
        "aligned": bool(row.get("aligned")),
        "lean": _vote_lean(vote),
...
def _audit_supplemental_evidence_row(key):
    ...
    return {
        "key": key,
        "gloss_cn": evidence_gloss_cn(key),
        "participation_status": status,
        "vote": 0.0 if status != "EXCLUDED" else None,
        "configured_weight": _audit_configured_weight(key),
        "reliability": 1.0 if status != "EXCLUDED" else None,
        "information": 0.0,
        "effective_weight": 0.0,
        "weighted_contribution": 0.0,
        "net_contribution_pct": 0.0,
        "aligned": False,
        "lean": "NEUTRAL" if status != "EXCLUDED" else None,
        "source_ref": _EVIDENCE_SOURCE_REF.get(key),
...
def _audit_strength_pct(decomp):
    strength = safe_float((decomp or {}).get("strength"))
    if strength is None:
        return None
    return int(round(max(0.0, min(1.0, strength)) * 100.0))


def _directional_bias(reasoning, conflict):
    score = safe_float((reasoning or {}).get("edb_score"))
    if score is None or abs(score) < 1e-12:
        return "NEUTRAL"
    suffix = "_WITH_DISAGREEMENT" if (conflict or {}).get("dissent") else "_CONFIRMED"
    return ("BULLISH" if score > 0 else "BEARISH") + suffix


def _dominant_conflict(conflict):
    aligned = conflict.get("aligned_keys") or []
    dissent = conflict.get("dissent") or []
    aligned_key = aligned[0] if aligned else None
    dissent_key = dissent[0].get("key") if dissent else None
    msg = None
    if aligned_key and dissent_key:
        msg = "主要同向证据 {0}，主要反向证据 {1}。".format(
            aligned_key, dissent_key)
    elif aligned_key:
        msg = "当前有效证据方向一致。"
    return {"aligned_key": aligned_key, "dissent_key": dissent_key,
            "explanation_cn": msg}


def _build_headline(card):
    conclusion = card.get("conclusion") or {}
    conflict = card.get("conflict") or {}
    return (str(CONFIG.get("asset", "BTC")) + "｜"
            + _brief_dir(conclusion.get("lean_cn")) + "｜"
            + _brief_action(conclusion.get("support_label")) + "｜置信"
            + _fmt_int(conclusion.get("confidence")) + "｜冲突"
            + _brief_pct(conflict.get("ratio")))


def _operator_focus(card):
    support = (card.get("conclusion") or {}).get("support_label")
    focus = ["同向有效证据是否继续增强", "反向证据是否回落"]
    if support == "WAIT_CONFIRMATION":
        focus.append("置信是否达到 50 的弱支持阈值")
    focus.append("降级或缺失数据源是否恢复")
    return focus


def _audit_integrity(record):
    ...
    return {
        ...
        "canonicalization": "SORTED_KEYS_COMPACT_UTF8_V1",
        "record_hash": "sha256:" + digest,
        "record_hash_scope": "entire_record_excluding_integrity",
        "redaction": {
            "version": "1.0.0",
...
Field semantics: docs/info接口语义文档.md in the gexmonitorapi repo. The default
deployment config now points at the strategy server; operators may still replace
gex_info_base_url / gex_info_token directly or via NRD_* runtime overrides.
"""
...
def _gex_info_endpoint(base):
    """Return the concrete /v1/info endpoint from a server root or full URL."""
...
class GexInfoAdapter:
    ...
    def _fetch_snapshot(self):
        ...
        base = _gex_info_endpoint(self.config.get("gex_info_base_url", ""))
...
class DemoRuntime:
    ...
    def _emit_push_self_test(self):
        ...
        # (no signal needed) to verify both JSONL persistence and short push.
        ...
        try:
            ...
            audit_card = dict(card)
            audit_card["is_synthetic"] = True
            record = build_audit_record(audit_card, self.config)
            wrote_json = self.recorder.write(
                self.config.get("signal_review_recorder_name", "signal_review"),
                record)
            body = _self_test_push_body(card, self.config, wrote_json)
        except Exception as exc:
            body = "【推送自检】样例渲染/写入异常：" + str(exc)
        fmz_push(body)
